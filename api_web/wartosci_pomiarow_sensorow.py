...
async def create_wartosc_pomiaru_sensora(wartosc_pomiaru_sensora: WartoscPomiaruSensora = Body(...)):
    ...
    db_miernik.zbior_wartosci_pomiaru_sensorow.insert_one(wartosc_pomiaru_sensora.dict(by_alias=True))
...
async def create_paczka_danych(id_paczki_danych: str, wartosc_pomiaru_sensora: WartoscPomiaruSensora = Body(...)):
    try:
        paczka_danych_result = db_miernik.zbior_wartosci_pomiaru_sensorow.find_one(ObjectId(id_paczki_danych))
...
async def get_wartosci_pomiaru_sensora():
    ...
    for wartosc_pomiaru in db_miernik.zbior_wartosci_pomiaru_sensorow.find():
...
async def get_wartosci_pomiaru_sensora_id(id: str):
    try:
        wartosc_pomiaru = db_miernik.zbior_wartosci_pomiaru_sensorow.find_one({"_id": ObjectId(id)})
        ...
        raise HTTPException(status_code=404, detail=f"Wartosc pomiaru sensora o id {id} nie znaleziono")
    except bson.errors.InvalidId:
...
async def delete_wartosc_pomiaru(id: str):
    delete_result = db_miernik.zbior_wartosci_pomiaru_sensorow.delete_one({"_id": id})
    print(delete_result)
