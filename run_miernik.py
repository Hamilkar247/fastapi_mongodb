...
import generateData
from models_miernik import User, db_testowo_miernik, dblist, StudentModel, UpdateStudentModel
...
async def create_student(student: StudentModel = Body(...)):
    ...
    new_student = db_testowo_miernik["students"].insert_one(student) #await przed prawa stroną rownania - wywoluje blad
    # await przed prawa stroną rownania - wywoluje blad
    created_student = db_testowo_miernik["students"].find_one({"_id": new_student.inserted_id})
...
async def list_students():
    students = []
    for student in db_testowo_miernik.students.find():
        #print(student)
...
async def show_student(id: str):
    if (student := await db_testowo_miernik["students"].find_one({"_id": id})) is not None:
        return student
...
async def update_student(id: str, student: UpdateStudentModel = Body(...)):
    ...
    if len(student) >= 1:
        update_result = db_testowo_miernik["students"].update_one({"_id": id}, {"$set": student})
        if update_result.modified_count == 1:
            if (
                update_student := db_testowo_miernik["students"].find_one({"_id": id})
            ) is not None:
                return update_student
    if (existing_student := db_testowo_miernik["students"].find_one({"_id": id})) is not None:
...
async def delete_student(id: str):
    delete_result = db_testowo_miernik["students"].delete_one({"_id": id})
...
async def delete_student():
    db_testowo_miernik['students'].drop()
